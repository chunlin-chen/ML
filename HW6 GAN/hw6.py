...
def cosine_beta_schedule(timesteps):
    ...
    """
    steps = torch.linspace(0, timesteps, timesteps, dtype=torch.float64)
    alphas = 0.5 * (1 + torch.cos(math.pi * steps / timesteps))
    betas = 1 - alphas / alphas[0]  # Normalize to start at 0 and end at 1
    return betas
...
def sigmoid_beta_schedule(timesteps):
    ...
    """
    steps = torch.linspace(0, timesteps, timesteps, dtype=torch.float64)
    betas = torch.sigmoid((steps - timesteps // 2) / (timesteps // 10))
    betas = betas / betas[-1] * 0.02  # Normalize to match the original DDPM range
    return betas
...
class Dataset(Dataset):
    def __init__(
        ...
        self.transform = T.Compose([
            ...
            T.Resize((image_size, image_size)),       # Resize to desired size
            ...
            T.RandomResizedCrop(size=image_size, scale=(0.8, 1.0)),  # Random cropping
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        num_samples = 25,
        results_folder = './results_strong_v3',
        split_batches = True,
    ...
    def inference(self, num=1000, n_iter=5, output_path='./submission_strong_v3'):
...
dim_mults = (1, 4, 8)        # The model size will be (channels, 2 * channels, 4 * channels, 4 * channels, 2 * channels, channels)

timesteps = 500            # Number of steps (adding noise)
beta_schedule = 'linear'
...
# ckpt = './model-55.pt'
ckpt = './results_strong_v3/model-100.pt'
trainer.load(ckpt)
trainer.inference()
