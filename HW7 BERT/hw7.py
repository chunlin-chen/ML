...
mode_name = 'hfl/chinese-roberta-wwm-ext-large'
model = AutoModelForQuestionAnswering.from_pretrained(mode_name).to(device)
...
class QA_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            ...
            noise  = random.randint(-50,50) # max = +75 ~ -75
            # paragraph_start = max(0, min(mid - self.max_paragraph_len // 2, len(tokenized_paragraph) - self.max_paragraph_len))
            paragraph_start = max(0, min(mid - self.max_paragraph_len // 2 + noise, len(tokenized_paragraph) - self.max_paragraph_len))
...
def evaluate(data, output):
    ...
    for k in range(num_of_windows):
        ...
        if prob > max_prob and end_index >= start_index:
            ...
            print("start:", start_index, " end:", end_index, " answer:", answer.replace(' ', ''))
...
num_training_steps = 3650
num_warmup_steps = 50
scheduler = get_scheduler(
    name="cosine",  # Can be "cosine", "constant", etc.
    optimizer=optimizer,
...
print("Saving Model ...")
model_save_dir = "saved_model_strong" 
